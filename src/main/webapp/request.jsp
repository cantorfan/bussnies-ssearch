...
<%
	...
	*/
	...
	System.out.println("search results:"+placesJsonStr);
	...
	int min = 10;
	System.out.println("size:"+placesJsonArray.length());
	for(int i=0; i<placesJsonArray.length() && i<min ; i++){
		JSONObject obj =(JSONObject) placesJsonArray.get(i);
		...
		System.out.println("detail request:"+detailUrl);
		...
		*/
		...
		JSONObject placeDetailJson = new JSONObject(placeDetailJsonStr);
		JSONObject placeDetail =  placeDetailJson.getJSONObject("result");
		String placeName = placeDetail.getString("name");
		
		responseJson += placeName;
		
		if((i+1)<min && (i+1)<placesJsonArray.length()){
			responseJson +=",";		
		}
